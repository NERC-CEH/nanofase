...
contains
    ...
    subroutine pfase_equilibrium_partition(pfas, volume_water, mass_solid, kd, phase)
        ...
        if (phase < 1 .or. phase > size(pfas,3) .or. phase == PFAS_AQ) return

        do i = 1, min(size(pfas,1), size(kd))
            denom = volume_water + max(0.0_dp, kd(i)) * mass_solid
            if (denom <= C%epsilon) cycle

            do f = 1, size(pfas,2)
                mtot = max(0.0_dp, pfas(i,f,PFAS_AQ) + pfas(i,f,phase))
                aq_new = mtot * volume_water / denom
                pfas(i,f,PFAS_AQ) = aq_new
                pfas(i,f,phase) = mtot - aq_new
            end do
        end do
    end subroutine pfase_equilibrium_partition


    subroutine pfase_kinetic_exchange(pfas, dt, k_ads, k_des, capacity, phase)
        real(dp), intent(inout) :: pfas(:,:,:)
        ...
        n = min(size(pfas,1), size(k_ads), size(k_des), size(capacity))

        do i = 1, n
            do f = 1, size(pfas,2)
                cap_remaining = max(0.0_dp, capacity(i) - pfas(i,f,phase))

                ads = min( &
                    pfas(i,f,PFAS_AQ), &
                    max(0.0_dp, k_ads(i)) * dt * pfas(i,f,PFAS_AQ) * max(0.0_dp, cap_remaining) &
                )

                des = min( &
                    pfas(i,f,phase), &
                    max(0.0_dp, k_des(i)) * dt * pfas(i,f,phase) &
                )

                pfas(i,f,PFAS_AQ) = max(0.0_dp, pfas(i,f,PFAS_AQ) - ads + des)
                pfas(i,f,phase)   = max(0.0_dp, pfas(i,f,phase)   + ads - des)
            end do
        end do
    end subroutine pfase_kinetic_exchange


    subroutine pfase_transform_network(pfas, dt, k_react, reaction_yield)
        ...
        ns = min( &
            size(pfas,1), size(k_react,1), size(k_react,2), &
            size(reaction_yield,1), size(reaction_yield,2) &
        )

        allocate(delta(size(pfas,1), size(pfas,2), size(pfas,3)))
        delta = 0.0_dp
        ...
        do i = 1, ns
            ...
            do f = 1, size(pfas,2)
                do p = 1, size(pfas,3)
                    loss = min(max(0.0_dp, pfas(i,f,p)), max(0.0_dp, pfas(i,f,p)) * frac_loss)
        ...
        pfas = max(0.0_dp, pfas + delta)
        deallocate(delta)
    end subroutine pfase_transform_network

end module PFASEProcessModule
