    ...
    contains
        ...
        function createBedSedimentLayer1(Me, &
            ...
            tr = Me%name // &
                "%createBedSedimentLayer1%pd_comp"                   !! trace message
                            ...
                            call fs1%create(Me%name, Me%pd_comp)     !! run constructor for this object
                            call move_alloc(fs1, &
            ...
            do x = 1, S                                              !! compute V_w for each size fraction using the sediment:water ratio
                associate (O => Me%colFineSediment(S)%item)          !! association for the FineSediment object we are working with
                    call r%addError(.errors. &
                        O%set(V_w_in = O%V_f() * fwr))               !! is used to compute the volume of associated water
                        if (r%hasCriticalError()) then               !! if a critical error has been thrown
                            call r%addToTrace(tr)                    !! add trace to Result
                            return                                   !! exit, as a critical error has occurred
                        end if
        ...
        function addSediment1(Me, S, F) result(r)
            ...
            associate(O => Me%colFineSediment(S)%item)
                ...
                Mf = V_f_added * F%rho_part(S)                       !! read in added mass - prevents multiple calls to object
                ...
                call r%addError(.errors. O%set(Vf_in = V_f_SC, &
                                               Vw_in = V_w_SC, &
                                           f_comp_in = t_comp &
                                              ) &
            ...
            call r%addError(.errors. F%set(Vf_in = add_V_f, &
                                           Vw_in = add_V_w &
                                          ) &
        ...
        function removeSediment1(Me, S, F, G) result(r)
            ...
            type(Result2D), allocatable :: r(:)                      !! The Result object. Element 1 = fine sediment that was removed; Element 2 = fine sediment that could not be removed
            ...
            character, len(*) :: tr                                  !! LOCAL error trace
            ! Function purpose
            ! -------------------------------------------------------------------------------
            ...
            associate (O => Me%colFineSediment(S)%item)
                ...
                allocate(r(2), stat = Me%allst)                      !! allocate Result object
                if (stat /= 0) then
                    r%addError = ErrorInstance(1, &
                                        "Allocation error", &
                                        [tr] &
                                              )                      !! if  allocation error thrown
                    return                                           !! critical error, so exit
                end if
                r%AddError(.errors. r(1)%item%set( &
                                    Vf_in = V_f_SC_r, &
                                    Vw_in = V_w_SC_r, &
                                    f_comp_in = O%f_comp &
                                              ) &
            ...
            r%AddError(.errors. r(2)%item%set( &
